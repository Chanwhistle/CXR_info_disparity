"""VLM dataset loading and CXR selection."""
...
from core.logging_utils import rank_zero_print
...
def normalize_race(value) -> str:
    """Normalize missing and unavailable race values to UNKNOWN."""
    if value is None or pd.isna(value):
        return "UNKNOWN"

    if value == "" or value in UNKNOWN_RACE_VALUES:
        return "UNKNOWN"
    return str(value)


class VLM_Dataset(Dataset):
    """Build chat inputs from selected modalities."""
    ...
    def __init__(self, args,
        ...
        # Detect the split from the path.
    ...
    def __getitem__(self, idx):
        ...
        # Select the best CXR. Admissions without an in-window study yield None.
        ...
        selected_img_data_path = selected_img_data[1] if selected_img_data else None

        if self.use_cxr_image and not selected_img_data_path:
            raise ValueError(
                f"No chest X-ray taken between admission and discharge for id {item['id']}, "
                "but an image modality was requested. Rebuild the dataset with "
                "preprocessing/build_lcd_cxr_dataset.py, which reports these as missing_images."
            )
    ...
    def _get_report_paths(self, img_data_path):
        """Return original and generated report paths."""
    ...
    def _load_images(self, mapped_image_path: str) -> list:
        ...
        # Hugging Face image processors expect PIL images or 0-255 arrays.
        # Passing a ToTensor() result makes Qwen rescale [0, 1] pixels again,
        # collapsing almost all image contrast around the normalized minimum.
        return [img]
    ...
    def _get_prompt(self, dn=None, images=None, rr=None, pi=None):
        """Build prompts for selected modalities."""
...
def merge_system_into_user(messages):
    """Fold the system turn into the user turn, preserving the prompt text."""
    system_texts = [
        message["content"] if isinstance(message["content"], str)
        else " ".join(part.get("text", "") for part in message["content"])
    ...
    for message in messages:
        ...
        for index, part in enumerate(content):
            if part.get("type") == "text":
                content[index] = {"type": "text", "text": f"{preamble}\n\n{part['text']}"}
                break
        merged.append({"role": message["role"], "content": content})
    return merged


def render_chat_template(processor, messages, _warned=[]):
    """Render a chat template, working around templates that ban system turns."""
    try:
        return processor.apply_chat_template(messages, tokenize=False)
    except Exception as error:
        # The Llama-3.2-Vision template raises when a prompt contains both a
        # system turn and an image. The instructions still have to reach the
        # model, so move them to the front of the user turn.
        if "system" not in str(error).lower():
            raise
        if not _warned:
            _warned.append(True)
            rank_zero_print(
                f"Chat template rejected the system turn ({error}); "
                "merging it into the user turn."
            )
        return processor.apply_chat_template(merge_system_into_user(messages), tokenize=False)


def custom_data_collator(processor, use_cxr_image=False, summary_type="plain"):
    """Build a processor-backed collator."""
    def collate_fn(examples: list) -> Dict[str, torch.Tensor]:
        ...
        for example in filtered_examples:
            texts.append(render_chat_template(processor, example["chat_template"]))
...
def load_hash2meta_dict(mapper_path, image_path):
    """Map note hashes to image metadata."""
    ...
    for metadata in img_path_parser['data']:
        ...
        if hash_key:
            hash2meta[hash_key] = {
                ...
                "race": normalize_race(metadata['debug_features'].get('RACE')),
...
class CXRDecisionTree:
    """Select the highest-priority recent CXR."""
